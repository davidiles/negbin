...
N_array = array(NA, dim = c(length(years), 4, max(obs_year_age)))
...
cat("
    ...
    #RECORD NUMBER OF BREEDERS EACH YEAR
    for (year in 1:Y){
...
params <- c("r","zero.B0","beta.fdepth",
            ...
            "surv.1.intercept","surv.2.intercept","surv.3.intercept",
            "surv.1.beta.cobble","surv.2.beta.cobble","surv.3.beta.cobble",
...
fullplot = plot_grid(p1,p2,p3,p4,cobble_plot, nrow=3)
print(fullplot)



mu.breeding.med = apply(out$sims.list$mu.breeding,2,function(x) quantile(x,0.500))
mu.breeding.lcl = apply(out$sims.list$mu.count[,,2] + out$sims.list$mu.count[,,3] + out$sims.list$mu.count[,,4],2,function(x) quantile(x,0.025))
mu.breeding.ucl = apply(out$sims.list$mu.count[,,2] + out$sims.list$mu.count[,,3] + out$sims.list$mu.count[,,4],2,function(x) quantile(x,0.975))
